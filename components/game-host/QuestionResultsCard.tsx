      ...
      <CardContent>
        ...
        <div className="space-y-3 mb-8">
          {question.answers.map((answer) => {
            ...
            const percentage = Math.round((answerCount / Math.max(1, totalValidAnswers)) * 100);

            return (
              <div key={answer.id} className="p-3 border rounded-lg">
                <div className="flex justify-between items-center mb-1">
                  <span className={`text-lg ${answer.isCorrect ? "font-semibold text-green-700 dark:text-green-400" : ""}`}>
                    {answer.answerText}
                  </span>
                  <span className="text-sm font-medium">
                    {answerCount} {answerCount === 1 ? "vote" : "votes"} ({percentage}%)
                  </span>
                </div>
